...
if !exists('g:NeatStatusLine_color_position')
    " White on Black
    let g:NeatStatusLine_color_position='guifg=#ffffff guibg=#000000 ctermfg=247 ctermbg=236 cterm=bold'
endif
...
if !exists('g:NeatStatusLine_color_line')
    " Pink on Black
    let g:NeatStatusLine_color_line='guifg=#ff00ff guibg=#000000 gui=bold ctermfg=207 ctermbg=236 cterm=bold'
endif

if !exists('g:NeatStatusLine_color_ycline')
    " Pink on Black
    let g:NeatStatusLine_color_ycline='guifg=#ff00ff guibg=#000000 gui=bold ctermfg=231 ctermbg=31 cterm=bold'
endif
...
if !exists('g:NeatStatusLine_color_ycinsert')
    " Black on Cyan
    let g:NeatStatusLine_color_ycinsert='guifg=#000000 guibg=#00ffff ctermfg=231 ctermbg=31 cterm=bold'
endif
...
function! ModeChanged(mode)
    ...
    elseif a:mode ==# "i"
        ...
        exec 'hi User5 '.g:NeatStatusLine_color_ycinsert
        exec 'hi User7 '.g:NeatStatusLine_color_ycline
    elseif a:mode ==# "r"
        exec 'hi User1 '.g:NeatStatusLine_color_replace
...
if has('statusline')
    ...
    function! SetStatusLineStyle()
        ...
        let &stl=""
        " mode (changes color)
        let &stl.="%1*\ %{Mode()} %5*".g:NeatStatusLine_separator
        " session name
        "let &stl.="%5* %{g:neatstatus_session} %5*".g:NeatStatusLine_separator
        " file path
        let &stl.="%5* %<%F "
        ...
        let &stl.="%5* %( %{&filetype} %)%5*".g:NeatStatusLine_separator." "
        " file format (eg. unix, dos, etc..)
        let &stl.="%5* %{&fileformat} ".g:NeatStatusLine_separator." "
        " file encoding (eg. utf8, latin1, etc..)
        let &stl.="%5* (%{(&fenc!=''?&fenc:&enc)} ".g:NeatStatusLine_separator." %)"
        " buffer number
        "let &stl.="BUF #%n ".g:NeatStatusLine_separator
        "line number (pink) / total lines
        let &stl.="%5* LN %7*%l%5*/%L\ (%p%%) %5*".g:NeatStatusLine_separator." "
        " percentage done
        "let &stl.="%5* (%p%%) ".g:NeatStatusLine_separator." "
        " column number
        let &stl.="%5* COL %7*%c%V %5*".g:NeatStatusLine_separator
